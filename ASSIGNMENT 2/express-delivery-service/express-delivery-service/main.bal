...
kafka:ConsumerConfig kafkaConsumerConfig = {
    ...
    topics: ["express-requests"]
};

kafka:ProducerConfig kafkaProducerConfig = {
    bootstrapServers: kafkaBootstrapServers,
    clientId: "express_delivery_producer"
};
...
public function main() returns error? {
    ...
    while true {
        ...
        foreach var record in records {
            ...
            DeliveryConfirmation confirmation = check processExpressDelivery(request, mongoClient);
            check producer->send({
...
function processExpressDelivery(CustomerRequest request, mongodb:Client mongoClient) returns DeliveryConfirmation|error {
...
function calculateEstimatedDeliveryTime(string pickupTime) returns string {
    // Simplified logic: assume express delivery takes 12 hours
...
function storeDeliverySchedule(DeliveryConfirmation confirmation, mongodb:Client mongoClient) returns error? {
    ...
    mongodb:Collection collection = check db->getCollection("express_delivery_schedules");
